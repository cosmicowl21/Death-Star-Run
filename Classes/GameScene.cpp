...
bool GameScene::init()
{
	...
	SimpleAudioEngine::getInstance()->playBackgroundMusic(Level);

    auto backgroundSprite = Sprite::create( "Android/Space.png" );
    backgroundSprite->setPosition( Point( visibleSize.width / 2 + origin.x, visibleSize.height / 2 + origin.y ) );
    
    this->addChild( backgroundSprite );

	auto DeathStar = Sprite::create("Android/DeathStar.png");
	backgroundSprite->setPosition(Point(visibleSize.width / 2 + origin.x, visibleSize.height / 2 + origin.y));

	this->addChild(DeathStar);
...
bool GameScene::onContactBegin( cocos2d::PhysicsContact &contact )
{
    ...
    if ( ( BIRD_COLLISION_BITMASK == a->getCollisionBitmask( ) && OBSTACLE_COLLISION_BITMASK == b->getCollisionBitmask() ) || ( BIRD_COLLISION_BITMASK == b->getCollisionBitmask( ) && OBSTACLE_COLLISION_BITMASK == a->getCollisionBitmask() ) )
    {
		...
		//SimpleAudioEngine::getInstance()->stopBackgroundMusic(Level);
    }
